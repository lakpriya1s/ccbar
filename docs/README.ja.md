...
- **2 行目** — **コンテキストウィンドウ**、**5 時間**のレート制限、**7 日間**のレート制限を示す色付きの使用量バーに加え、従量課金（pay-as-you-go）の支出を示す任意の**オーバーレッジ**バー。それぞれにリセットまでのカウントダウンが付きます。

バーは満ちるにつれて 🟢 緑 → 🟡 黄 → 🔴 赤 と変化するため、残りの余裕がどれくらいあるかをひと目で把握できます。

コアのバーに加えて使用状況を管理するのに役立つ、いくつかの追加機能があります。
...
- **バーンレート警告** — 現在のペースがリセット*前*に制限を使い切る見込みのとき、ccbar はそのバーに `⚠ <time>` の推定を付加します。デフォルトでオンになっています。`ccbar config` で無効にできます。
- **オーバーレッジバー** *(オプトイン)* — 5 時間・7 日間のプランクォータとは独立して、従量課金（pay-as-you-go）／追加使用分の支出を追跡する別の `over` バーです（Pro/Max アカウントでは両方を同時に抱えることもあります）。デフォルトではオフです。`ccbar config` で有効にできます。
...
| `CCBAR_SHOW_OVERAGE` | `0`     | 従量課金（オーバーレッジ）バーを表示（`1`/`0`）。                    |
| `CCBAR_SHOW_COST`   | `0`     | 1 行目に実行中のセッションコストを表示（`1`/`0`）。                 |
| `CCBAR_SHOW_BURN`   | `1`     | ペースが制限を早く使い切る場合に警告（`⚠ <time>`）（`1`/`0`）。 |
