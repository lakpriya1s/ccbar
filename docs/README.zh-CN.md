...
- **第 2 行** — 分别针对你的**上下文窗口**、**5 小时**速率限制和**7 天**速率限制的彩色用量条，外加一个可选的**超额**用量条用于显示按量付费支出，每条都带有距离重置的倒计时。

用量条会随着填充程度由 🟢 绿 → 🟡 黄 → 🔴 红 变化，让你一眼就能看出还剩下多少余量。

还有一些附加功能，帮助你管理核心用量条之外的用量：
...
- **消耗速率警告** — 当你当前的使用节奏预计会在某个限制*重置之前*就将其耗尽时，ccbar 会在该用量条后附加一个 `⚠ <time>` 估算。默认开启；可在 `ccbar config` 中关闭。
- **超额用量条** *(需手动开启)* — 一个独立的 `over` 用量条，用于跟踪按量付费/额外用量支出，与 5 小时和 7 天的套餐额度互相独立（Pro/Max 账户可以同时拥有两者）。默认关闭；可在 `ccbar config` 中启用。
...
| `CCBAR_SHOW_OVERAGE`| `0`     | 显示按量付费/超额用量条（`1`/`0`）。                    |
| `CCBAR_SHOW_COST`   | `0`     | 在第 1 行显示实时会话花费（`1`/`0`）。                 |
| `CCBAR_SHOW_BURN`   | `1`     | 当你的使用节奏会提前耗尽某个限制时发出警告（`⚠ <time>`）（`1`/`0`）。 |
