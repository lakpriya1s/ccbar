...
- **2번째 줄** — **컨텍스트 창**, **5시간** 사용 한도, **7일** 사용 한도에 대한 색상 사용량 막대와, 사용량 기반 결제(pay-as-you-go) 지출을 위한 선택적 **초과 사용량** 막대이며, 각각 재설정까지 남은 시간을 함께 표시합니다.

막대는 채워질수록 🟢 초록 → 🟡 노랑 → 🔴 빨강으로 바뀌므로, 남은 여유가 얼마나 되는지 한눈에 알 수 있습니다.

핵심 막대 외에도 사용량을 관리하는 데 도움이 되는 몇 가지 추가 기능이 있습니다:
...
- **소진 속도 경고** — 현재 속도로는 한도가 재설정되기 *전에* 소진될 것으로 예상될 때, ccbar는 해당 막대에 `⚠ <time>` 추정치를 덧붙입니다. 기본적으로 켜져 있으며, `ccbar config`에서 비활성화하세요.
- **초과 사용량 막대** *(선택 활성화)* — 사용량 기반 결제(pay-as-you-go)/추가 사용량 지출을 추적하는 별도의 `over` 막대로, 5시간 및 7일 요금제 한도와는 독립적입니다(Pro/Max 계정은 두 가지를 동시에 가질 수 있습니다). 기본적으로 꺼져 있으며, `ccbar config`에서 활성화하세요.
...
| `CCBAR_SHOW_OVERAGE` | `0`     | 사용량 기반 결제(초과 사용량) 막대 표시 여부(`1`/`0`).           |
| `CCBAR_SHOW_COST`   | `0`     | 1번째 줄에 실시간 세션 비용 표시 여부(`1`/`0`).                 |
| `CCBAR_SHOW_BURN`   | `1`     | 현재 속도로 한도가 일찍 소진될 때 경고(`⚠ <time>`) 표시 여부(`1`/`0`). |
